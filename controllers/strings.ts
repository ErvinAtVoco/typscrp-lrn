...
router.get("/random/:nr1/:nr2", (req: Request, res: Response) => {
    const nr1 = Number(req.params.nr1);
    const nr2 = Number(req.params.nr2);
    res.send((Math.floor((Math.random()* nr2) + nr1)).toString());
});

router.get("/sunna/:aasta", (reg: Request, res: Response) => {
    const aasta = Number(reg.params.aasta);
    const current = new Date().getFullYear()
    res.send("Olenevalt sünnipäevast oled " + ((current-aasta).toString()) + " aastat vana.")
})
